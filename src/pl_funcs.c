 ...
 */
void
pltsql_ns_push(const char *label)
{
	if (label == NULL)
		label = "";
	pltsql_ns_additem(PLTSQL_NSTYPE_LABEL, 0, label);
}
 ...
 */
void
pltsql_ns_additem(int itemtype, int itemno, const char *name)
{
	PLTSQL_nsitem *nse;
	...
	nse = palloc(sizeof(PLTSQL_nsitem) + strlen(name));
 ...
 */
const char *
pltsql_getdiag_kindname(int kind)
{
	switch (kind)
	{
